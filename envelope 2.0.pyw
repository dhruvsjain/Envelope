...
def login_and_send(to, msg):
    ...
    try:
        f = open('eap.envelope', 'rb')
        d = pickle.load(f)
        from_addr = d.keys()[0]
        password = d[from_addr]
        port_no = d['Port number']
...
##        task_finish = tk.Button(frame1, text = 'finish task'.upper(), relief='flat', bg=bglight_colour, fg=button_colour, font=button_font, padx=20, pady=10, command = self.finish_on_click)
##        task_not_complete = tk.Button(frame1, text = 'task not completed'.upper(), relief='flat', bg=bglight_colour, fg=button_colour, font=button_font, padx=20, pady=10, command = self.nc_on_click)        
...
##        task_finish.grid(row = 4, column = 0)
##        task_not_complete.grid(row = 5, column = 0)
...
##    def finish_on_click(self):
##        self.list_of_contacts.itemconfig('active', fg = green)
##
##    def nc_on_click(self):
##        self.list_of_contacts.itemconfig('active', fg = red)
...
class schedule_screen:
    ...
    def on_click(self):
        ...
        msg = 'Subject: ' + sub + '\n' + self.box.get('1.0','end')
##        to = self.to_txt.get().replace(' ', '') + ';'
        if self.to_txt.get() not in ['', ' ']:
            to = self.to_txt.get().split(';')
        else:
            to = []
        for key in d:
            to += [d[key]]
        ...
        # a list is created from the string, that is to
        # smtplib can take multiple email ids in the form of a list
        if m:
            self.root.destroy()
            for contact in d:
                msg1 = msg.replace('%name', contact)
                time_is_right(hrs, mins, d[contact], msg1)
